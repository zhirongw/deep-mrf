...
function gradcheck.numeric_gradient_crit(f, x, df, eps)
  df = df or 1.0
  eps = eps or 1e-8
  local seq_length = #x
  assert(#x[1] == 4)
  ...
  for t = 1,seq_length do
    local gmm = {}
    for g=1,4 do
      table.insert(gmm, torch.Tensor(x[t][g]:size()):fill(0))
    end
    table.insert(dx_num, gmm)
  end
  for t = 1, seq_length do
    for g=1,4 do
      local n = dx_num[t][g]:nElement()
      local x_flat = x[t][g]:view(n)
      local dx_num_flat = dx_num[t][g]:view(n)
      for i = 1,n do
        local orig = x_flat[i]
