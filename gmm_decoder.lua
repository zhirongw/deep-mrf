...
function layer:updateOutput(input)
  ...
  local p = 2
  if self.pixel_size == 3 then
    local g_cov = self.cov_tanh:forward(input:narrow(2,p*self.num_mixtures*self.pixel_size+1,self.num_mixtures*self.pixel_size))
    p = p + 1
  ...
  local g_w = self.w_softmax:forward(input:narrow(2,p*self.num_mixtures*self.pixel_size+1,self.num_mixtures))
...
function layer:updateGradInput(input, gradOutput)
  ...
  if self.pixel_size == 3 then
    ...
    dg_cov = self.cov_tanh:backward(input:narrow(2,p*self.num_mixtures*self.pixel_size+1,self.num_mixtures*self.pixel_size), dg_cov)
  ...
  local dg_w = self.w_softmax:backward(input:narrow(2,p*self.num_mixtures*self.pixel_size+1, self.num_mixtures), gradOutput[3])
  -- border is a single scalar indicator
